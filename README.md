...
Originally based on [mcp-canvas-lms](https://github.com/DMontgomery40/mcp-canvas-lms), ported to run as a remote MCP server with multi-user support. The Canvas tools are now source-owned in this project.
...
- **Source-owned tools** — tool registration, Canvas API methods, and Canvas types live directly in this repository
...
src/
  ...
  canvas-client.ts     ← HTTP infrastructure plus source-owned Canvas API methods
  canvas-tools.ts      ← Read-only MCP tool registrations with Zod schemas
  canvas-types.ts      ← Canvas entity interfaces
  types.ts             ← Public type exports + CanvasAPIError
scripts/
  generate-direct-canvas-openapi.ts ← Direct Canvas OpenAPI generator for private GPT Actions
```
