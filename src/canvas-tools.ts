...
export function registerAllTools(server: McpServer, client: CanvasClient, options: RegisterToolsOptions = {}): void {
  ...
  if (shouldRegisterTool("canvas_get_upcoming_assignments", readOnly)) server.registerTool(
    ...
    {
      description: "Get upcoming assignment-related events from Canvas's upcoming events feed",
      inputSchema: {
  ...
  if (shouldRegisterTool("canvas_list_assignments_for_active_courses", readOnly)) server.registerTool(
    "canvas_list_assignments_for_active_courses",
    {
      description: "List assignments with due dates in a time range across all active courses",
      inputSchema: {
        start_date: z.string().describe("Start of the time range (yyyy-mm-dd or ISO 8601)"),
        end_date: z.string().describe("End of the time range (yyyy-mm-dd or ISO 8601)"),
        limit: z.number().describe("Maximum number of assignments to return after sorting by due date").optional(),
      },
      annotations: toolAnnotations("canvas_list_assignments_for_active_courses"),
    },
    async (args) => {
      try {
        return ok(
          await client.listAssignmentsForActiveCourses(args.start_date, args.end_date, args.limit),
          formatter,
        );
      } catch (error) {
        return fail(error);
