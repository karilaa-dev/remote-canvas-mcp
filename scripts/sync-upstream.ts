...
function generateToolRegistration(tools: ToolDef[]): string {
  ...
  const lines: string[] = [
    ...
    `const MUTATING_TOOL_NAMES = new Set<string>([${Array.from(mutatingTools).map((name) => JSON.stringify(name)).join(", ")}]);`,
    `const DESTRUCTIVE_TOOL_NAMES = new Set<string>([${Array.from(destructiveTools).map((name) => JSON.stringify(name)).join(", ")}]);`,
    `const IDEMPOTENT_TOOL_NAMES = new Set<string>([${Array.from(idempotentTools).map((name) => JSON.stringify(name)).join(", ")}]);`,
    ``,
  ...
  for (const tool of tools) {
    ...
    lines.push(`  if (shouldRegisterTool(${JSON.stringify(tool.name)}, readOnly)) server.registerTool(`);
